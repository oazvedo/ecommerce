...
namespace api.Domain.Interfaces
{
    public interface IEmpresaRepository : IRepositoryBase<Empresa>
    {
        ...
        Task<(IEnumerable<Empresa> Items, int TotalCount)> GetFiliaisAsync(Guid empresaPaiId, int page, int pageSize);
        Task<bool> AdicionarUsuarioAsync(Guid empresaId, Guid usuarioId);
        Task<bool> DeleteComCascadeAsync(Guid empresaId);
        Task<Empresa?> UpdateCamposAsync(Guid id, string nome, string cnpj, string responsavel, Guid responsavelId, string telefone, api.Domain.Enums.EmpresaTipo tipo, bool status, Guid? empresaPaiId);
    }
}